...
function PackageListPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [packagesResponse, bookingsResponse] = await Promise.all([
          ...
          axios.get("/api/booking/getBookings"),
        ]);

        const fetchedPackages = Array.isArray(packagesResponse.data.packages)
          ? packagesResponse.data.packages
          : [];
        const fetchedBookings = Array.isArray(bookingsResponse.data.bookings)
  ...
  const isPackageBooked = (packageId) => {
    if (!checkInDate || !checkOutDate) return false;
    const relevantBookings = bookings.filter(
      (booking) =>
        booking.package._id === packageId &&
        booking.status === "confirmed" &&
        new Date(booking.checkInDate) <= new Date(checkOutDate) &&
        new Date(booking.checkOutDate) >= new Date(checkInDate)
    );
    const pkg = packages.find((p) => p._id === packageId);
    return pkg && relevantBookings.length >= pkg.roomType.totalRooms;
  };

  const handleBookNow = (packageId) => {
    // Redirect to PackageViewPage with the package ID
              ...
              >
                ...
                <div className="package-price" style={{ textAlign: "right" }}>
                  ...
                  <button
                    ...
                    disabled={isPackageBooked(pkg._id)}
                    style={{
                      backgroundColor: isPackageBooked(pkg._id) ? "#aaa" : "#219652",
                      color: "white",
                      border: "none",
                      padding: "5px 10px",
                      borderRadius: "4px",
                      cursor: isPackageBooked(pkg._id) ? "not-allowed" : "pointer",
                      marginTop: "10px",
                    }}
                  >
                    {isPackageBooked(pkg._id) ? "Fully Booked" : "Book Now"}
                  </button>
                </div>
              </div>
            ))}
